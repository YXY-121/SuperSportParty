...
func (dispatcher Dispatcher) ReadData() (msg common.Message, err error) {
	buf := make([]byte, 10240)
	...
	if err != nil {
		fmt.Println("server read data login data error: %v", err)
	}
...
func (dispatcher Dispatcher) SendData(data []byte) (err error) {
	...
	if writeLen != 4 || err != nil {
		fmt.Println("send data length to server error: %v", err)
		return
	}
	...
	if err != nil {
		fmt.Println("send data to server error: %v", err)
		return
	}
	return
}
